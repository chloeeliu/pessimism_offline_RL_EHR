...
def summarize_rows(rows: list[LinearRunMetrics]) -> dict[str, float]:
    return {
...
def summarize(
    baseline_rows: list[LinearRunMetrics],
    pessimistic_rows: list[LinearRunMetrics],
    betas: list[float],
    ...
    for size in dataset_sizes:
        ...
        best_beta_by_size[str(size)] = {
            ...
            "best_feature_novelty": float(best_stats["mean_chosen_feature_novelty"]),
        }

    return {"baselines": baselines, "pessimistic": pessimistic, "best_beta_by_size": best_beta_by_size}


def save_outputs(output_dir: Path, baseline_rows: list[LinearRunMetrics], pessimistic_rows: list[LinearRunMetrics], summary: dict[str, object], resolved_config: dict[str, object]) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    all_rows = baseline_rows + pessimistic_rows
    with (output_dir / "metrics.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(all_rows[0].to_dict().keys()))
        writer.writeheader()
        for row in all_rows:
...
def make_value_panels(output_dir: Path, summary: dict[str, object], dataset_sizes: list[int], betas: list[float]) -> None:
    ...
    for index, size in enumerate(dataset_sizes):
        ...
        ax.axhline(baselines["greedy"][str(size)]["mean_value"], color="#c03d3d", linestyle="--", label="greedy")
        if "support_masked" in baselines:
            ax.axhline(baselines["support_masked"][str(size)]["mean_value"], color="#2f9e44", linestyle=":", label="support_masked")
...
def make_diagnostic_panels(output_dir: Path, summary: dict[str, object], dataset_sizes: list[int], betas: list[float]) -> None:
    pessimistic = summary["pessimistic"]
    beta_keys = [f"{beta:.3f}" for beta in betas]
    ...
    diagnostics = [
        ...
        ("mean_chosen_feature_novelty", "Feature Novelty", "Novelty proxy"),
    ]
    ...
    for beta in betas:
        for num_episodes in dataset_sizes:
            for seed in range(args.seeds):
                ...
                pessimistic_rows.extend([row for row in rows if row.method == "pessimistic"])
                if not baseline_emitted:
                    baseline_rows.extend([row for row in rows if row.method != "pessimistic"])
        baseline_emitted = True

    summary = summarize(baseline_rows, pessimistic_rows, betas, dataset_sizes)
    resolved_config = {
        "task": args.task,
